...
| `tools/brand-surface.mjs` | 品牌棘轮（私有化批次 C）：25 个产品面文件里不许再出现上游 npm 名/平台包名/上游 registry/用户可见 "opencode" 字符串；两份 README 的每处 opencode 必须落在署名行（fork/upstream/anomalyco/MIT/上游/血缘/**包路径**）上；另加全树 18,652 处血缘计数（排除 fork 自己的血缘文档与 vendored 内核）——**只许减不许增** |
