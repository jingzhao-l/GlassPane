...
public enum PermissionGuide {
    ...
    public static func instruction(
        ...
        switch kind {
        case .accessibility:
            action = "已让 daemon 申请「辅助功能」并打开对应面板：请为 \(subjectName) 打开开关，开启后本窗口会自动变绿。"
        case .inputMonitoring:
            action = "已让 daemon 申请「输入监控」并打开对应面板：请为 \(subjectName) 打开开关（首次开启需重启 daemon 生效）。"
        case .screenRecording:
            action = "已让 daemon 申请「屏幕录制」并打开对应面板：请为 \(subjectName) 打开开关，随后需重启 daemon 生效。"
        case .developerTools:
            action = "开发者工具（Apple Events）无系统总开关：运行需要 LLDB attach 的操作时系统会询问是否允许控制，选允许即可。"
        }
