...
| M3 | `@iterate/kernel` 进 fork | 包在 fork 内被 M1/M2 的模块 import；绑定形态＝**带溯源的 vendored 镜像 + `file:` 依赖**（同 `mcp-shell` 先例） | 不占 patch 名额（依赖不是我们的工具面代码；`tool-surface` 按溯源清单把它算作依赖） | registry 上没有（`npm view` → 404、`private: true`、无 license）**但不是阻塞**：不单独发布是 P6 §13 / P4 §33.2 的既成决定，发布跟随 iterate monorepo 策略。本文早先"发 npm 是第 0 号动作"的说法已订正，见 §1-4/§7/§9-3 |
