# GlassPane 5.8 —— 项目综述文档

> 来源：ChatGLM 对话记录（https://chatglm.cn/share/FFb2jssD）
> 提取时间：2026-09-14
> 文档版本：v1.0（基于 GlassPane 5.7 综述复制迭代修订）
> 迭代时间：2026-09-14
> 前身：5.7（9 项审查修订 R32–R40）；5.6（13 项审查修订 R19–R31）；5.5（18 项审查修订 R1–R18）
>
> **5.7 → 5.8：实施级深化（R41–R45），不改变架构形态与战略结论；P0 walking skeleton 与 kernel Phase A 进入实施**
...
### 5.8 修订（R41–R45，新增：实施级深化）

| # | 修订 | 要点 |
|---|---|---|
| R41 | **Engine local socket 协议 v0 规格** | 帧格式（换行分隔 JSON）、八方法表（hello/attach/act/observe/assert_element/diagnose/last_evidence/shutdown）、GP_E 错误码表、安全边界（socket 文件 0600/父目录 0700/单帧上限 4 MiB/方法白名单/参数类型校验）——详见《GlassPane P0 实施规格》§3 |
| R42 | **evidence 包 schema v0.1-draft 落地** | JSON Schema 真源（$id 固定为 kernel schema 命名空间）+ Swift Codable / TS zod 双绑定；C35 测试双侧就位（两侧测试套件对**同一组真值 fixtures** 独立做 decode→encode→规范化比较）；draft→freeze 生命周期起算（C2 spike 真实 app 数据回填后冻结） |
| R43 | **kernel Phase A 宿置决策** | @iterate/kernel 暂宿 GlassPane 仓库（Phase A 零风险纯新代码原则的落地形态）；迁入 iterate monorepo 的触发条件=Phase B 两壳消费前，以保留 git 历史的方式迁移 |
| R44 | **MCP 壳 P0 零运行时依赖决策** | stdio 协议层手写（JSON-RPC 2.0 + MCP initialize/initialized/ping/tools 语义，完整实现而非桩）；官方 TS SDK 采用推迟至 P1 工具面扩张时重新评估——依据：P0 工具集仅 6 个、协议面小，手写层可全量单测且消除依赖面 |
| R45 | **仓库结构与可测性边界** | monorepo：`engine/`（SwiftPM 5.9+，macOS 13+）、`kernel/`、`mcp-shell/`（node ≥18）；AX 适配层运行时需辅助功能权限（不可单测，规格声明其验收方式=真机冒烟），纯逻辑（协议编解码/九分类判定/tree digest/pixel diff/evidence codec）100% 单测覆盖 |
...
| **5.8** | **实施级深化（socket 协议/错误码/schema 落地/kernel 宿置/零依赖 MCP 壳/可测性边界 R41–R45）+ P0 walking skeleton 与 kernel Phase A 实施启动** |
...
**5.7 → 5.8 的变更性质**：实施级契约深化（协议/schema/错误码/仓库结构/依赖决策），无架构级变更；P0 walking skeleton 与 kernel Phase A 按既有门控进入实施——kernel Phase A 仍不进入 P0/P1 关键路径（与 5.7 §10.2 声明一致）。
...
| 工具面 A：MCP Server 壳 | **TypeScript**（推荐） | 选择 | MCP 是 JSON-RPC 协议、语言无关；官方 SDK 覆盖 TS/Python/Java/Kotlin/C#/Go，无官方 Swift SDK。TS 壳仅做协议翻译（stdio/HTTP ↔ local socket），验证语义全部留在 Swift daemon，符合"薄适配层 <10%"铁律。备选：Swift 手写 MCP 消息层（消除进程跳转，列为 P3 优化项）。**P0 实施形态（5.8 R44）：协议层手写（零运行时依赖），SDK 采用在 P1 重新评估** |
...
| **共享内核 @iterate/kernel（消费侧）** | **TypeScript**（自研，npm 依赖） | 选择（战略，5.7 R32） | GlassPane 两个 TS 壳与 iterate 生态项目共同消费的语义内核（schema/审计链映射/事务原语/公共校验器），定位与门控见 §8.5。**npm 依赖不占 Patch 集名额，不计入壳代码量预算（以 fork 自有代码计）**。**Phase A 宿置：暂宿 GlassPane 仓库（5.8 R43），Phase B 前迁入 iterate monorepo** |
...
**P0 工具子集（5.8 新增，R44）**：P0 落地六类原语中的五个——会话管理（gp_attach）、感知（gp_observe）、行动（gp_act）、断言（gp_assert_element）、诊断（gp_diagnose + gp_last_evidence）；快照/恢复类原语按 P0 范围口径不实现（P1 进入，见 §10.2 walking skeleton 定义）。工具级契约（参数/返回/错误映射）见《GlassPane P0 实施规格》§6。
...
**P0 分类器能力边界（5.8 新增，R45）**：Z5 单通道下可用信号=AX 树 digest（树变与否）、像素 diff（画面变与否）、进程存活、AX 响应性（ping）。P0 分类器可判定：T0（通道故障）/ T1（崩溃）/ T2（假死）/ T3（操作确认但树与像素均无变化）/ T6（树变画面不变）；T4/T5/T7/T8/T9 需 handler/state 探针或时序信号，P0 输出 `INCONCLUSIVE`（诚实边界：requires-probe-signals，进 P1），不做猜测性归类。
...
**P0 实施范围（5.8 R38/R45）**：P0 仅实现 Z5（纯 AX+像素黑盒通道）；Z1–Z4.5 为 P1。Z5 通道内的像素捕获使用 `CGWindowListCreateImage`（macOS 14 起标记弃用，P0 可用）；ScreenCaptureKit 迁移列 P1 与权限 onboarding 一起做（R36 联动）。
...
**P0 熔断口径（5.8 新增，R41）**：P0 熔断级绑定通道健康度——0=正常；1=单通道降级（像素捕获失败或 AX 部分超时）；2=AX act 失败但通道存活；3=通道故障（与 T0 同源）。熔断级字段 `circuitBreaker.level` 进每个 evidence 包（与 5.5 语义一致），性能型熔断（操作耗时触发）P1 进入。
...
**Phase A 宿置与实施（5.8 R43/R45 补充）**：Phase A 的 @iterate/kernel 暂宿 GlassPane 仓库 `kernel/` 目录（独立 npm 包，包名/导出面按正式包设计）；迁入 iterate monorepo 的触发条件=Phase B 两壳消费前，以保留 git 历史的方式迁移（`git filter-repo`/subtree 均可，届时按 iterate monorepo 结构定）。Phase A 首发范围（本仓库内即刻实施）：evidence 包 schema v0.1-draft、decision log entry schema v0.1-draft、配方配置 schema v0.1-draft（三份 JSON Schema 真源 + zod 绑定 + C35 roundtrip 测试）；invariant 引擎/维度系统/事务原语的 TS 实现属 iterate 侧迭代节奏，随迁移进入。
...
- **C35（5.7 新增，R33）**：kernel schema 双语言绑定 roundtrip 一致性——JSON Schema 真源，Swift Codable / TS zod 双绑定，跨界样本 roundtrip 通过率 100%；为 kernel v0.1 冻结与 Phase B 入口的前置条件。**（5.8 R42 实施落地：双侧测试就位于本仓库 engine 与 kernel 测试套件，对同一组真值 fixtures 运行）**
...
5. **AX 权限摩擦（5.8 新增，R41/R45 关联）**：AX 适配层运行时依赖辅助功能权限授予，无权限时 P0 循环不可运行——这是首次运行体验的最大摩擦点。对冲：daemon 对权限缺失返回结构化错误码（GP_E_AX_UNAVAILABLE）+ agent 可执行补救指引（R36 失败模式指引）；onboarding 权限引导流（§5.14）为 P0 交付的一部分（命令行形态）。
...
（5.5 六项保留，5.6 新增两项，5.7 新增三项，5.8 新增三项：）
...
| kernel Phase A 宿置 | **暂宿 GlassPane 仓库 kernel/ 目录，Phase B 前迁入 iterate monorepo（5.8 R43）** | Phase A 零 fork 改动、纯新代码；本仓库先行保证 schema 草案与 C35 测试即刻可跑；迁移以保留 git 历史方式进行 |
| MCP 壳 P0 运行时依赖 | **手写 stdio 协议层（零运行时依赖）（5.8 R44）** | P0 工具面小（6 工具），手写层完整实现且可全量单测，消除依赖面；官方 TS SDK 在 P1 工具面扩张时重新评估 |
| P0 像素捕获 API | **CGWindowListCreateImage（接受弃用警告）（5.8 R41 关联）** | P0 可用且实现最小；ScreenCaptureKit 迁移与权限 onboarding 在 P1 一起做，避免 P0 同时引入新权限流程与新 API 学习成本 |
...
## 18. P0 实施规格索引（5.8 新增，R41–R45）

实施级契约全集收录于独立规格文档 **《GlassPane P0 实施规格 v1.0》**（specs/GlassPane_P0_实施规格_v1.0.md），综述按 R39 分工不重复展开，仅做索引：

| 规格章节 | 内容 | 综述锚点 |
|---|---|---|
| §2 仓库结构与工具链 | monorepo 布局、语言/工具链版本、依赖钉扎与许可证 | R45 |
| §3 Engine local socket 协议 v0 | 帧格式、八方法表（参数/返回/异常）、GP_E 错误码表、安全边界 | R41 |
| §4 evidence 包 schema v0.1-draft | 字段表、枚举、双语言绑定关系、C35 测试设计、fixtures 真值 | R42 |
| §5 kernel Phase A 包规格 | @iterate/kernel 导出面、目录结构、发布策略（暂宿→迁移） | R43 |
| §6 MCP 工具契约 P0 子集 | 六工具（gp_attach/gp_observe/gp_act/gp_assert_element/gp_diagnose/gp_last_evidence）参数/返回/错误映射 | R44 |
| §7 测试与验收 | 纯逻辑单测矩阵、C35 双侧 roundtrip、AX 冒烟流程 | R45 |
| §8 安全与边界 | socket 权限、输入校验、帧上限、无遥测声明 | R41 |
| §9 降级与诚实边界 | Z5 信号缺失→INCONCLUSIVE、AX 权限错误、CG API 弃用 | R45/§5.3 |
| §10 版本与迁移 | schema draft→freeze 生命周期、协议版本化策略 | R42 |
...
\*本文档为 GlassPane 5.8 实施级深化版。5.7 的架构形态、战略结论与 R32–R40 修订全部原样保留；5.8 的修订集中于实施级契约（R41–R45：socket 协议与错误码、evidence schema v0.1-draft 落地、kernel 宿置决策、MCP 壳零运行时依赖决策、仓库结构与可测性边界）与实施启动（P0 walking skeleton + kernel Phase A）。实施契约全集见《GlassPane P0 实施规格 v1.0》（§18 索引）。\*
...
**5.8 使用提示**：① 实施以《GlassPane P0 实施规格 v1.0》为契约真源，本综述 §18 仅做索引——实施中发现契约需要变更时，先改规格再改代码，两者不得漂移；② §16 新增三行决策（kernel 宿置/MCP 零依赖/CG 弃用 API）是 P0 的边界决策，P1 评审时逐一回顾；③ C35 测试已双侧就位，schema 任何字段变更必须双侧 fixtures 同步修改——这是 draft 阶段唯一的纪律成本。
