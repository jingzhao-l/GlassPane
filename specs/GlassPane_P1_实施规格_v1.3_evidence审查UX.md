# GlassPane P1 实施规格 v1.3 — evidence 审查 UX（开发者审计面）

- 契约真源角色：P1 第四子项目的实施契约（P1 首批 = SCK 迁移 + 屏幕录制权限 onboarding，见 `GlassPane_P1_实施规格_v1.0_SCK迁移.md`；批二 = 快照/恢复原语 + 性能型熔断，见 `GlassPane_P1_实施规格_v1.1_快照恢复与性能熔断.md`；批三 = GUI 设置面板，见 `GlassPane_P1_实施规格_v1.2_GUI设置面板.md`，其全部验收项已达标且正文不变；本规格基于 v1.2 的章节结构与约束体系迭代，不删减既有内容，仅追加批次四范围——evidence 审查 UX）
- 版本：v1.3
- 日期：2026-09-16
- 关联决策：综述 §5.14 evidence 审查 UX（R36）、PRD US-10（开发者回归审计）、P0 §3.4 GP_E 错误码表、FR-04 诊断报告四段（PATH/ANOMALY/EVIDENCE/NEXT）、evidence schema v0.1-draft（R42）、mcp-shell 工具面（R44/§6 工具契约）；本规格批次四续编 §10–§13（由 v1.3-draft 顶部 §1–§4 移编重编号而来）

---

## 变更说明（相对 P1 v1.2 批次）

前三批收敛了 daemon 能力（SCK/快照/熔断）与开发者入口（CLI → GUI 设置面板）。本批次把开发者回归审计的一屏体验落地为**可导出、可回放的 evidence 审查面**（综述 §5.14 / PRD US-10）：

- engine 新增 `EvidenceReportGenerator`（纯逻辑）：把一条 `EvidencePack`（已含操作/归因/污染/熔断/信号字段，schema v0.1-draft 既存）渲染为**四段诊断时间线 HTML**（PATH/ANOMALY/EVIDENCE/NEXT）与**结构化 Markdown** 两种导出形态，供 Harness 壳/MCP 面消费同一内容。
- mcp-shell 新增两工具 `gp_export_evidence`（按 operationId 导出）与 `gp_recent_reports`（最近 N 条四段报告聚合视图）；两者只做"取 evidence → 渲染模板"的纯逻辑，不涉及 kernel schema 变更。
- 不改变 evidence schema、不改变 daemon 方法表（渲染完全发生在壳层/工具层，复用 `last_evidence`/`diagnose` 既有返回）。
- 与 P1 v1.2 §6 GUI 面板的关系：面板是"实时权限 daemon 状态"，本批次是"离线会话回放"——面板启动 daemon，审查工具审计 evidence；两者为同一开发者支持面的两个视图，不互相依赖。

批次四范围要点：

- 渲染产物为 deterministic 字符串（HTML/Markdown 同一份 evidence 输入 → 同一份输出），纯逻辑可单测（golden 快照断言）。
- HTML 含 operationId 锚点、归因/污染/熔断标签、信号展开区、诊断四段；Markdown 为紧凑版（终端/对话框友好）。
- 错误场景：查无该 operationId → `GP_E_NO_EVIDENCE`（既有错误码）原样透传；渲染永不因个别字段缺失崩溃（缺省为空段落）。

> **版本结构说明（v1.3-draft → v1.3）**：draft 阶段曾把批次四内容置顶（§1–§4），与下方复刻的 v1.2 正文 §1–§9 产生重复编号。正式版已消除冲突：批次四内容移编至文件末尾 **§10–§13**（由原 §1–§4 逐节重编号），v1.2 正文 §1–§9 逐字保留不变。三处设计缺口（diagnostics 兜底语义、会话内 operationId 来源、`GP_E_NO_EVIDENCE` 新错误码）在 §10.3/§11/§13 补述。
...
## 10. evidence 审查 UX（工具面 A 扩展：八工具 → 十工具）

### 10.1 定位

对应综述 §5.14 / PRD US-10「开发者回到桌面 → 第一屏=失败断言汇总+待裁决队列 → 按 operationId 证据卡片回放会话（可按归因强度过滤）」。P0/P1 前三批的 evidence 只能通过 `gp_last_evidence` 拿原始 JSON；本批次提供**人读视图**（诊断时间线 HTML / 紧凑 Markdown），让不解析 JSON 的开发者也能审计一次会话。

### 10.2 engine 新增（纯逻辑渲染器）

`engine/Sources/GlassPaneEngine/EvidenceReportGenerator.swift`：

- `renderHTML(pack: EvidencePack, diagnostics: String?) -> String`：四段时间线 + 信号/归因/污染/熔断展开；operationId 锚点；确定性输出（无时间戳注入——capturedAt 以 ISO8601 固定格式原样呈现，不做运行时格式化）。
- `renderMarkdown(pack: EvidencePack, diagnostics: String?) -> String`：紧凑式（标题 + 关键字段表 + 四段 blocks）。
- `reportTitle(pack:) -> String`：`operationId · measure · level` 人读标题；measure 回退链 = diagnosis class → assertion property → action rawValue；level 标签 = normal/degraded/act_failed_channel_alive/channel_fault（数字确定性 `%g` 格式化，去除浮点尾 `.0`）。
- 两者共用 `evidenceSummaryLines(pack:) -> [String]`（归因/污染/熔断/信号摘要行），避免两视图字段漂移。
- 缺省安全性：`actions`/`signals`/`attribution` 等任一可选字段缺失时渲染为空段落/`—`，不抛错。

### 10.3 mcp-shell 工具扩展（工具表追加两工具）

| 工具 | 定位 | inputSchema | 成功返回（content[0].text） | 失败 |
|---|---|---|---|---|
| gp_export_evidence | 按操作 ID 导出一条证据的可读视图 | `{operationId: string(必填), format?: enum[html, markdown] 默认 markdown}` | 渲染后的 HTML/Markdown | GP_E_NO_EVIDENCE（原码+补救）/ GP_E_NOT_ATTACHED / GP_E_BAD_PARAMS |
| gp_recent_reports | 最近 N 条 evidence 的聚合四段报告 | `{limit?: integer 1–20 默认 5, format?: enum[html, markdown] 默认 markdown}` | 逐条标题 + 四段摘要 + 分隔线 | GP_E_NOT_ATTACHED / GP_E_BAD_PARAMS / GP_E_NO_EVIDENCE（无记录/全部不可达） |

- 数据链路：`gp_export_evidence` → 发 `last_evidence {operationId}` 帧给 daemon → 取 `EvidencePack` JSON → kernel `parseEvidencePack` 强校验（对应 mcp-shell `parseEvidenceFrame`，沿用工具面既有强校验路径，工具契约见 mcp-shell `tools.ts` 规格区块）→ 渲染器 → 返回文本。
- `gp_recent_reports` → 逐 operationId 复用同一链路（上限 20 条，防大包）。
- 渲染器纯 TS 无需 kernel schema 新导出：复用既有 `parseEvidencePack` 与 evidence zod（`additionalProperties: false` 维持）。

> **设计缺口补述 1：`gp_recent_reports` 的 operationId 来源（会话内跟踪）**。daemon 方法表冻结（不可新增"枚举历史"方法），因此聚合视图的 operationId 清单由 **mcp-shell 会话内 `EvidenceAuditSession`** 承担：默认工具路径在每次成功返回后 `record(raw)`，递归有界抓取 JSON 中字符串型 `operationId`/`evidenceId` 键（深度 ≤3，环形保留最近 `AUDIT_HISTORY_LIMIT=20` 条）；成功 `attach` 时清空会话（镜像 daemon"换 app 清 evidence 历史"语义）。daemon 侧环形历史（64 条）淘汰或 daemon 重启导致部分 operationId 不可达时，`gp_recent_reports` **跳过该条目并附 note**（`skipped N unreachable entry/entries`），不全报失败；全部条目不可达时返回 `GP_E_NO_EVIDENCE` 及补救指引。

> **设计缺口补述 2：`diagnostics` 兜底语义**。渲染器签名带 `diagnostics: String?`（为 Harness 壳侧手工诊断预留），对齐规则：pack 内嵌 `diagnosis.report.evidence` 存在时优先使用并忽略 diagnostics；report 缺失而 diagnostics 非空时，diagnostics 作为 EVIDENCE 段的 **preformatted 兜底**（`<pre>`/代码块形态，明确是外部输入而非诊断推断）；两者皆缺时输出 `—` 占位。两个审计工具不携带 diagnostics（不手工撰写诊断），调用渲染器时传 nil——EVIDENCE 段缺省仍走"report.evidence → `—`"。

### 10.4 与既有面的关系（无冲突）

| 面 | 现状 | 本批次 | 关系 |
|---|---|---|---|
| GUI 设置面板 | 权限/daemon 实时状态 | 不变 | 两个独立视图：面板=当前运行态，审查=历史回放 |
| gp_last_evidence | 原始 JSON | 不变 | 审查工具在其之上加渲染层（不改变原始返回） |
| diagnose | 四段 JSON | 不变 | 审查 HTML 复刻四段文案，不接管 diagnose 语义 |

---

## 11. 双语言联动与 schema 影响（本批次无 schema 变更）

| 关注面 | 影响 |
|---|---|
| evidence-pack schema | 无变更（`additionalProperties: false` 维持；渲染器只读） |
| decision-log / recipe-config schema | 无变更 |
| kernel zod 绑定 | 无新增导出（复用 `parseEvidencePack` 与既有 evidence zod） |
| kernel fixtures | 无新增（无新 schema 类型）；既有 C35 双侧 roundtrip 不受影响 |
| Swift Codable | 无 evidence 变更；渲染器为纯函数（输入 EvidencePack → 输出字符串） |

> **设计缺口补述 3：`GP_E_NO_EVIDENCE` 为新增错误码（非既有）**。规格 draft 原文写"（既有错误码）"，但核查 P0 §3.4 错误码表并无此码——原实现将"查无 operationId"归并到 `GP_E_NO_OPERATION`，语义混淆（operation 存在但无 evidence ≠ 无 operation）。本批次落地：engine 错误码表新增 `noEvidence = "GP_E_NO_EVIDENCE"`（remedy = "produce evidence first (act/assert_element/diagnose), or check the operationId matches a recorded operation"），`lastEvidence` 的"查无 operationId / 空历史"两处改抛新码；`diagnose` 查无保持 `GP_E_NO_OPERATION` 不变；mcp-shell `errors.ts` 常量同步。仅改错误码表不改方法表，符合"方法表冻结"约束，实现与 P1-D4 测试同步验证。

---

## 12. 可测性与验收

- **engine 单测**（无 GUI 权限，CI 可跑）：构造固定 `EvidencePack` fixture → golden 断言 HTML/Markdown 含关键段（PATH/ANOMALY/EVIDENCE/NEXT、operationId、归因标签、contaminated、circuitBreaker level）；缺字段 fixture → 不崩溃且含 `—` 缺省；确定性：两次渲染字节相等。
- **mcp-shell 单测**：新工具注册/转发/格式枚举校验/缺 evidence 错误映射/会话跟踪/聚合与跳弃；复用既有 `node:test` 基建。
- **无头 CI 降级**：无 daemon 时工具层错误路径（GP_E_NOT_ATTACHED）可测，渲染黄金样本在 engine 侧覆盖。

| 编号 | 验收项 | 通过标准 | 状态 |
|---|---|---|---|
| P1-D1 | 渲染器确定性 | 同一条 EvidencePack：两次 renderHTML/renderMarkdown 输出字节完全相等；含四段标题（PATH/ANOMALY/EVIDENCE/NEXT）与 operationId | ✓ 已实现并单测通过（2026-09-16，`EngineP1Batch4Tests` golden/确定性 3 用例） |
| P1-D2 | 缺字段容错 | 任意可选字段缺失（如 signals.pixelDiff 为 null / diagnosis 缺失）渲染不抛错，输出 `—` 缺省 | ✓ 已实现并单测通过（2026-09-16，minimalPack 缺省 + HTML 转义用例） |
...
1. **渲染器只读 evidence、不产生新语义**：HTML 里的四段文案直接复刻 diagnose 既存输出，不重写诊断推断；若 evidence 无 diagnose 关联，四段区显示相应空段占位（如实声明）。
2. **gp_recent_reports 并发上限 20**：受会话跟踪环形上限 `AUDIT_HISTORY_LIMIT=20` 与 daemon `history` 内存保留策略约束（与 `last_evidence` 同生命周期）；超出上限的批量导出属后续大数据量批次。
3. **确定性渲染不含时间戳合成**：capturedAt 以 evidence 内原值呈现，渲染过程零随机（hash/random 均不引入），保证 golden 快照稳定。
4. **HTML 为受控模板**：所有 evidence 字段值经 HTML 转义（`&`/`<`/`>`/`"`/`'` 五字符全集），杜绝注入（安全红线 4 对齐）；渲染器不执行任何 JS。
5. **双语言渲染镜像防漂移**：Swift `EvidenceReportGenerator`（canonical）与 TS `evidence-report.ts`（类型镜像）共享同一结构契约（标题/四段/摘要行/转义/`—` 占位符）与同一 golden 期望，规格强制"同一 commit 双改"，任何单侧改动须双侧 golden 同步通过。