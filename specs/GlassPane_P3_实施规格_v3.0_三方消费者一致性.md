# GlassPane P3 实施规格 v3.0 — 三方消费者一致性（C28/C35）

- 契约真源角色：P3 第一批次的实施契约（P3 一批为 v3.0 C28/C35 三方消费者一致性；P2 全套 v2.0–v2.1、P1 全套 v1.0–v1.6 验收项已达标且正文保留不变；本规格基于 v2.1 的章节结构与约束体系迭代，不删减既有内容，仅追加批次一范围——C28/C35 三方消费者一致性：kernel API ↔ Swift Codable ↔ mcp-shell 消费者 三点回环契约测试 + 失败三分类归因）
- 版本：v3.0
- 日期：2026-09-17
- 关联决策：综述 §5.3 **R33/C35**（证据 schema 双语言契约：JSON Schema 为唯一真源、Swift Codable 与 TS zod 双绑定、roundtrip 契约测试）、**R34/C28 扩展**（MCP 契约一致性测试扩展为 kernel 消费者一致性、失败三分类归因：kernel API / fork 适配胶水 / 上游）、§8.5 kernel Phase B 门控"P1 出口 + C28/C35 达标"、P0 规格 §7.2（C35 双侧测试套件对同一组真值 fixtures 独立 roundtrip）
