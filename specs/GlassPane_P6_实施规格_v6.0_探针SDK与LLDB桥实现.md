...
### §11.1 补录（2026-09-20，用户 GUI 实测反馈轮）

用户实测"开发者工具权限探测不到授权状态"，且点名界面文案不得写"§x.x/口径/不猜结论"
一类开发者自辩话术。排查出三个叠加的真实缺陷（面板 801aaac 形态）：

1. **launchctl 路径硬编码 `/usr/bin/launchctl`** —— 本机真值在 `/bin/launchctl`
   （真机发现 F11）。面板四类 launchd 动作（席位重探、授权申请、调试能力验证、
   daemon 重启）全部 `Process.run` 抛"file doesn't exist"，而旧代码把失败吞成
   `return nil` → 用户侧症状="点了没反应/恒未验证"。修复：`Launchctl.path`
   一次性解析（/bin→/usr/bin 探测，均为纯函数可测），四处命令构造共享；单测
   `testLaunchctlPathResolvesToRealBinary` 把"不许硬编码猜路径"钉死。
2. **一次性任务的轮询等待在主 actor 上执行** —— 调试能力验证预算 260s（重探 6s）
   直接把面板主线程冻死，AX 读不到、UI 无响应、约 10s 后被系统看门狗处理。修复：
   `Task.detached` 移出主线程 + `runOneShot` 失败路径打 stderr 诊断；另加
   `NSTemporaryDirectory()` 目录守卫（非 sandbox 进程该目录可能不存在，原子写
   直接抛错是第 2 个静默失败点）。
3. **实测结论上不了徽标** —— 验证成功后卡片徽标仍恒"未验证"，结论只在 caption
   行，用户读不到"授权状态"。修复：developerTools 卡有带时刻结论时徽标改按实测
   状态显示「可用（实测）」/「被拒绝（实测）」（caption 行保留时刻与两侧明细；
   daemon 换 pid 时旧结论作废，不冒充新进程状态）。`.p4i4_smoke.py` 的 dev 卡
   合法徽标集合同步为 {未验证, 可用（实测）, 被拒绝（实测）}。

**文案口径（产品约束，永久生效）**：面板用户可见字符串不得含规格引用（§/P1 v1.2）
与开发者自辩话术（不猜/不伪造/口径/如实/冒充/席位/裸二进制/非实时读数…），
"诚实边界"由行为保证（缺数据就显示未验证），不靠文案自我声明。本轮已全量改写
四卡用途/降级说明、引导横幅、错误提示、重启指引、探测 caption；机器锚点
...
**合流说明（main 90f04d4）**：§11.10 S15 查明的信号哑火根因（ARC 释放未持有源 +
