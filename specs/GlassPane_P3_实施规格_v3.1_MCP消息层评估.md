# GlassPane P3 实施规格 v3.1 — Swift 手写 MCP 消息层评估

- 契约真源角色：P3 第二批次的实施契约（P3 一批为 v3.0 C28/C35 三方消费者一致性；P2 全套 v2.0–v2.1、P1 全套 v1.0–v1.6 验收项已达标且正文保留不变；本规格基于 v3.0 的章节结构与约束体系迭代，不删减既有内容，仅追加批次二范围——Swift 手写 MCP 消息层评估/决策：消除进程跳转的选项复审，落地结论为维持 TS 壳现状并定义触发重估条件）
- 版本：v3.1
- 日期：2026-09-18
- 关联决策：综述 §16 已收敛决策档"**MCP Server 壳用 Swift 手写协议层 —— 否决（列为 P3 优化项）**"（原因原文：官方无 Swift SDK，TS 生态 SDK 成熟；壳仅做协议翻译不承载语义，跨语言成本一次性且微小）、§16 决策行"**MCP 壳 P0 运行时依赖 —— 手写 stdio 协议层（零运行时依赖）（5.8 R44）**：官方 TS SDK 在 P1 工具面扩张时重新评估"、综述 §8.5 消费者矩阵（R34，GlassPane MCP 壳为 kernel 四消费者之一）
