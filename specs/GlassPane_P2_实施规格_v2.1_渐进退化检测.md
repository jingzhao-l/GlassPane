# GlassPane P2 实施规格 v2.1 — 渐进退化检测（T9）

- 契约真源角色：P2 第二批次的实施契约（P2 一批为 v2.0 C33；P3 及后续见 v3.0+；P1 全套 v1.0–v1.6 验收项已达标且正文保留不变；本规格基于 v2.0 的章节结构与约束体系迭代，不删减既有内容，仅追加批次二范围——渐进退化检测：主线程 ping + 内存曲线斜率 + 句柄计数三信号联合判定）
- 版本：v2.1
- 日期：2026-09-17
