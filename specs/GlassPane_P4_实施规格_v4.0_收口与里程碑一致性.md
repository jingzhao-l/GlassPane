# GlassPane P4 实施规格 v4.0 — 收口与里程碑一致性检查（实施规格系列最终版）

- 契约真源角色：P4 收口批次的实施契约（P3 一批为 v3.0 C28/C35 三方消费者一致性、批二为 v3.1 Swift 手写 MCP 消息层评估、批三为 v3.2 LLDB 桥形态评估；P2 全套 v2.0–v2.1、P1 全套 v1.0–v1.6 验收项已达标且正文保留不变；本规格基于 v3.2 的章节结构与约束体系迭代，不删减既有内容，仅追加批次四范围——P4 收口：规格系列版本收口、全量验收清点、里程碑一致性检查与遗留面留档）
- 版本：v4.0（实施规格系列最终版）
- 日期：2026-09-18
- 关联决策：综述 §10.2 里程碑表（P0 最薄垂直切片 / P1+ C34 与分发公证 / P2+ C33 与 T9 / P4 未列标准）、综述关键断言（C28 扩展 / C33 / C34 / C35）、综述 §16 决策档（像素捕获 API / kernel 宿置 / MCP 壳零依赖 / Swift 手写协议层否决 / LLDB 桥形态）、综述 R45 可测性边界（AX/真机面人工冒烟口径）
...
## 批四变更说明（P3 v3.2 → P4 v4.0）

v4.0 是实施规格系列的收口批次。综述 §10.2 里程碑主体仅排至 P3（5.8 未给 P4 增补标准），P4 在计划中的义务是收尾动作而非新能力：**规格系列版本收口、全量验收清点、里程碑一致性检查、遗留面留档**。本批对 P0→P3 全部 13 份实施规格做系统复审（§30.2 逐表清点），核对综述里程碑/断言/决策行的落地一致性（§30.3–§30.5），并宣布版本收口（§30.6）。

- **评估范围**：清点与一致性审计，不引入任何新实现；为真相优先，对"待办"验收项逐一查原文归类（§30.2 注明性质）；
- **评估方法**：全量规格验收表逐行统计 + 综述 §10.2/断言清单/§16 决策行逐条对照 + 铁约束与测试基线复核（基线取各批运行记录，不重跑）；
- **结论（本批正式落地）**：自动验收面全绿（320 行 × 302 ✓，18 项未标 ✓ 全部为"真机冒烟（可选人工项）"，非实现缺口）；里程碑 P0–P3 一致性全部达成；spec 系列正式收口于 v4.0；
- **落地形态**：本批为**评估/决策批次，零代码变更**（规格-only）；验收以清点完整性与一致性判定为准（§31 P4-D1..D5）。
...
## 30. P4 收口与里程碑一致性检查（P3 v3.2 → P4 v4.0 新增）

### 30.1 规格系列终态清单

实施规格系列共 13 份 + 综述 3 份 + PRD 1 份（specs/ 目录实存）：

| 系列 | 文件 | 批次主题 |
|---|---|---|
| P0 | P0 实施规格 v1.0 | walking skeleton：socket 协议 v0 / 八方法表 / GP_E 错误码 / 单通道完整循环 |
| P1 | v1.0 SCK 迁移 → v1.6 evidence 归档生命周期（7 份） | 能力扩展七批：SCK 迁移 / 快照恢复+熔断 / GUI 设置 / evidence 审查 UX / 多项目与配方 / evidence 落盘 / 归档生命周期 |
| P2 | v2.0 并发归因污染防护、v2.1 渐进退化检测（2 份） | 能力扩展两批：C33 操作权互斥与污染标注 / T9 三信号联合判定 |
| P3 | v3.0 三方消费者一致性、v3.1 MCP 消息层评估、v3.2 LLDB 桥形态评估（3 份） | 工程收口三批：C28/C35 闭环 / "消除依赖"两项复审义务兑现 |
| P4 | **v4.0 收口（本份）** | 版本收口 + 一致性检查 + 遗留留档（实施规格系列**最终版**） |

### 30.2 全量验收清点总表（2026-09-18 逐表复审，正文保留不变）

口径说明：统计各文件**全文所有验收表**合计（继承部分与新增部分一并计入）；P0/P1 v1.0 早期批次为三列表格式（无独立"状态"列），其能力由后继批次验收与真机交付逐项覆盖，不计为"未 ✓ 缺口"。

| 规格 | 验收行数 | ✓ | 未 ✓（编号｜原文归类） |
|---|---|---|---|
| P0 v1.0 | 7 | —（三列表格式） | 验收项 A1–A7 无状态列；能力由后继批次与真机交付覆盖 |
| P1 v1.0 | 6 | 1 | A1–A5 无状态列；A6 内联 ✓ |
| P1 v1.1 | 7 | 6 | P1-B7｜真机冒烟（可选人工项） |
| P1 v1.2 | 13 | 11 | P1-B7、P1-C6｜真机冒烟（可选人工项） |
| P1 v1.3 | 19 | 16 | P1-B7、P1-C6、P1-D6｜真机冒烟（可选人工项） |
| P1 v1.4 | 8 | 8 | 无 |
| P1 v1.5 | 16 | 16 | 无 |
| P1 v1.6 | 23 | 23 | 无 |
| P2 v2.0 | 31 | 31 | 无 |
...
| P3 v3.1 | 50 | 50 | 无 |
| P3 v3.2 | 55 | 55 | 无 |
| **合计** | **320** | **302 ✓** | **18 项未标 ✓，全部为"真机冒烟（可选人工项）"（v1.1–v1.3，需 AX 权限/GUI——综述 R45 声明的真机验收面，不进 CI 单测通道，非实现缺口）** |

### 30.3 里程碑一致性矩阵（综述 §10.2 vs 实施）

| 阶段 | 综述 §10.2 关键标准 | 实施落地 | 判定 |
|---|---|---|---|
| P0 | 最薄垂直切片（单一验证通道 × 一条完整循环） | daemon + 真机冒烟 + MCP 壳六工具早期落地（fa537b4/0e02454 批次，方法表/错误码/安全边界按 P0 规格） | ✓ 一致 |
| P1 | C34 档 1 恢复保真率 ≥95% 且分级恢复时间达标；分发公证 | 快照/恢复原语 + 性能型熔断落地（v1.1–v1.2，ffwd 重演侧基础度量入 C34 验收入口后移声明）；分发公证属发布流程事项（归档/签名/公证在 archive 阶段执行，非仓库代码可验收） | ✓ 一致（公证列为发布期事项，见 §32.2） |
| P2 | C33 并发归因污染检出率 >95%、误杀率 <5%；T9 长会话泄漏注入检出 | v2.0 全 31 项 ✓（操作权互斥/空闲检测/污染标注，合成场景集统计口径）；v2.1 全 39 项 ✓（泄漏注入检出与无症状零误报双面断言） | ✓ 一致 |
| P3 | 5.8 无增补标准（承接 R33-C35、R34-C28 扩展的实施义务） | v3.0 C28/C35 三点回环 46 ✓（kernel fixtures 真值四层路径 + decision-log Swift 绑定）；v3.1 MCP 消息层复审关闭优化项；v3.2 LLDB 桥形态复审维持 Python | ✓ 一致 |
| P4 | §10.2 未列（里程碑主体仅至 P3） | 本批：收口 + 一致性检查 + 清点汇报 | ✓ 一致（按计划文件收尾义务） |

### 30.4 关键决策行兑现核对（综述 §16 + R41–R45）

| 决策行 | 结论 | 实施状态 |
|---|---|---|
| P0 像素捕获 API CGWindowListCreateImage | 接受（弃用但可用） | P1 SCK 迁移落地（eeabcd5/a7e82ad），含 macOS 13 SCStream 单帧捕获桥 |
| kernel Phase A 宿置（暂宿 kernel/） | 暂宿仓库，Phase B 前迁入 iterate monorepo | kernel/ 已落地（d542494，schema 真源 + zod 绑定 + C35 测试）；迁移触发条件未到，保持暂宿 |
| MCP 壳 P0 运行时依赖 | 手写 stdio 协议层（零运行时依赖，R44） | 已落地（fa537b4 stdio 壳 + 六工具） |
| GlassPane 消费 kernel 形态 | 两 TS 壳消费、Swift 不进 kernel | mcp-shell 经 `@iterate/kernel` 消费（0842477 起 kernel schema 强校验）；Swift 侧经 C35 双绑定对齐（v3.0 补齐 decision-log 绑定） |
| MCP 壳用 Swift 手写协议层 | 否决（列为 P3 优化项） | **v3.1 复审兑现：维持 TS 壳，关闭优化项，三守卫条件** |
| LLDB 桥形态 | Python 脚本 + SB API（刚性）；C++ plugin 为后续优化项 | **v3.2 复审兑现：维持 Python，C++ 不自动激活，三守卫条件** |
| 断言 C33 / C34 / C35 / C28 扩展 | 见各阶段验收 | 全部在对应批次验收表 ✓ 落地（§30.2/§30.3） |

### 30.5 铁约束遵守复核（收口时点逐项核对）

- **daemon/socket 方法表冻结**：八方法表（v0）自 P1 v1.0 的合法扩展后**冻结**；v2.0/v2.1（C33/T9）为 EngineCore 内部协调 + evidence 标注，零新帧；P3 三批零协议变更。MCP 工具面新增（v1.4 八→十一、后 13）是工具面扩展，不是 socket 帧新增，与各批铁约束口径一致。
- **错误码冻结**：GP_E 表最后扩展为 v1.4（GP_E_PROJECT_LIMIT / GP_E_NOT_FOUND）；此后批次（T9/C33/P3 全批）**零新增错误码**。
- **kernel schema 零修改**：C35 生效以来 fixtures/zod/JSON Schema 全部保持 kernel 原样；v3.0 显式承诺并履行"不修改 kernel 任何文件"。
- **依赖许可**：依赖面（ajv/zod/@iterate/kernel 等）全为宽松许可；d74342e 曾以升级 ajv 8.17.1→8.20.0 修复 moderate ReDoS（依赖治理实例）。
- **可测性边界（R45）**：纯逻辑 100% 单测（协议编解码/九分类判定/tree digest/pixel diff/evidence codec）；AX/CGEvent/LLDB/真机冒烟面按 R45 归真机人工验收。

### 30.6 版本收口声明

1. 实施规格系列（P0 v1.0 → P3 v3.2 → **本 v4.0**）正式收口，**v4.0 为最终版**；P0–P3 全部批次验收达成（自动验收面 100% ✓，人工冒烟项如实留档 §32.1）。
2. 后续新需求/新阶段**另起新规格系列文档**，不在本系列追加（避免复制继承文件持续膨胀）。
3. 综述 5.8 保持快照不改写；本收口审查判定的里程碑一致性结论以本规格 v4.0 为权威记录。

---

## 31. 收口验收

- 本批为评估/决策批次：交付物为清点事实 + 一致性判定 + 收口声明，无代码变更，无新增测试用例；既有基线沿用（engine 234 / mcp-shell 65 / kernel 47，全为各批运行记录）。
- 验收口径：清点完整性（13 份规格逐表统计，无遗漏）、一致性判定逐阶段可溯（§30.3 每阶段给出落地点）、收口声明显式（§30.6）。

| 编号 | 验收项 | 通过标准 | 状态 |
|---|---|---|---|
| P4-D1 | 全量验收清点 | 13 份实施规格验收表逐行统计（320 行）；未标 ✓ 项逐条核实性质并归类 | ✓ 已完成（2026-09-18，§30.2；18 项未标 ✓ 全部为"真机冒烟（可选人工项）"，无实现缺口） |
| P4-D2 | 里程碑一致性 | P0/P1/P2/P3 逐阶段对照综述 §10.2 标准给出判定（含 C33/C34/C35/C28 扩展示例） | ✓ 已完成（2026-09-18，§30.3；四阶段全一致，P1 公证项归发布期） |
| P4-D3 | 决策行兑现核对 | 综述 §16/R41–R45 关键决策行逐行给实施状态；已收口复审项（v3.1/v3.2）标注结论 | ✓ 已完成（2026-09-18，§30.4） |
| P4-D4 | 铁约束遵守复核 | 方法表/错误码/kernel schema/依赖许可/可测性边界逐项复核无违反 | ✓ 已完成（2026-09-18，§30.5） |
| P4-D5 | 版本收口 | spec 系列最终版显式声明；新需求另起系列的规定入档 | ✓ 已完成（2026-09-18，§30.6） |

---

## 32. 遗留与边界（诚实声明）

1. **人工冒烟项 18 项**（v1.1–v1.3 的 B7/C6/D6 系）：真机 AX 权限/GUI 人工验收面，随真机验收阶段执行；与综述 R45 声明一致（该类面不进 CI 通道），非实现缺口。
2. **分发公证（notarization）**：综述 P1 里程碑含"分发公证"，但签名/公证在 archive 阶段执行（需开发证书与 Apple 公证服务），属**发布流程事项而非仓库代码可验收**——仓库侧义务以可构建/可签名配置为准，公证合规留给发布批次。
3. **无头环境的运行边界（R23/R45）**：AX 查询（需 GUI 会话）、CGEvent 输入监控（需输入监控 TCC）、LLDB attach（需开发者工具权限）在无 GUI runner 下不可用，其真机冒烟面如实待办，不以模拟数据充验收。
4. **kernel Phase B 迁移**：@iterate/kernel 迁移 iterate monorepo 的触发条件（Phase B 两壳消费前）未到，保持暂宿本仓库；迁移时按"保留 git 历史"方式执行（R43）。
5. **P1 评审遗留观察点**：R44"官方 TS SDK 在工具面扩张时重新评估"观察点已随 v3.1 记录在案（工具面已达 13）；是否切换属独立决策批次，评估权保留。
...
| `engine/.signal_smoke.py` | 信号收尾回归闸（P1 v1.2 §11.10）：起隔离 daemon（独立 engine/probe socket，`--no-c33`）→ 发 SIGTERM 与 SIGINT → 要求**自行退出且退出码 0、两个 socket 文件被 unlink**；产物缺失 SKIP（exit 0） | 无 TCC 依赖 | ✓ SIGNAL SMOKE OK（2026-09-20 修复后）；对照复跑旧构建两案均"6s 内未退出"，即此前四次 SIGTERM 哑火的确定性复现 |
