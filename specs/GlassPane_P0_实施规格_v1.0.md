...
**镜像数量从一处变两处（2026-09-25，M3 落地后）**：`tools/sync-kernel.sh` 现在有两个目标——`--target=repo` 仍是本仓 `kernel/`（mcp-shell 经 `file:../kernel` 消费）；`--target=fork` 把同一份 canonical 同步成 `harness/glasspane-harness/packages/opencode/vendor/kernel/`（`src`/`schemas`/`fixtures` 三类，不带 `test/`，因为 fork 不跑 node:test 那套）。**归属方向没变**：canonical 只有 `iterate-skill/kernel` 一个编辑入口，两处都是单向同步目标，谁都不许手改——fork 那处由 `harness/tools/kernel-vendor.mjs` + 金样 `harness/contracts/kernel-vendor.json`（逐文件 sha256 + canonical 的 repo/branch/ref/version）机器守住，CI 每跑。**为什么 fork 不直接用 `file:` 依赖**：kernel 不单独发布是既成决定（P6 §13），而 fork 走 `git subtree split` 独立成仓后 `../../kernel` 必然失联；`file:` 出包即失联正是当年 mcp-shell 要 bundle 内联的同一个原因。**为什么放在 `packages/opencode` 里面而不是顶层新包**：bun 的依赖挂在各包自己的 `node_modules`，顶层新目录解析不到 `zod`，而给它补 package.json 就得重跑整棵 fork 的 `bun install`。代价如实记一条：**同一份内核源码从此跑在两个 zod 大版本上**（canonical 钉 3.25.76，fork catalog 4.1.8），第一次 fork 侧 typecheck 就抓出一处 zod 4 前向不兼容（`z.record` 单参），修在内核。守卫脚本自身的两次失误（参数化时把删除检测的 `-an` 写成 `-n`；回滚用 `git clean` 在首次 vendoring 时删掉了自己刚写的镜像）与补上的反向控制记在 `harness/glasspane-harness/SYNCLOG.md`。

**How to apply**：改内核只在 canonical；两处镜像都用 `tools/sync-kernel.sh --target=repo|fork` 同步，且后者的第三道守卫会跑 fork 自己的 `bun test` + `tsgo --noEmit`（缺 bun 时**明说哪两道闸没跑**，不把"命令找不到"当代码红）。
