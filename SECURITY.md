...
- 仓库为 `jingzhao-l/GlassPane`（public，MIT）。**开启私密漏洞报告后**（Settings → Security →
  ...
  `https://github.com/jingzhao-l/GlassPane/security/advisories/new`。
- 若该入口尚不可用，退化路径是开一个普通 issue，只写**症状与影响**，把复现细节与产物留在私下一方，
  由维护者私信补充。
- 无论走哪条：**不要**在公开 issue 里粘贴令牌、`~/.glasspane/` 下的日志原文、证据包 JSON、
  导出报告或截图。这些文件里可能含他人界面上的账号、姓名、邮件内容。
...
**边界（如实声明）**：本机同权限进程。跨用户、跨网络、以及已被 root 的机器都不在防护范围内，
相关报告通常会被判定为 out of scope。
...
- 辅助功能：读取任意 app 的 AX 树（含控件标题与属性文本），并通过 `AXUIElementPerformAction` 对任意
  app 执行动作（点击/按下等）。当前实现不写 `AXValue`、不合成 CGEvent 键鼠事件。
- 输入监控：全局事件 tap，用于分辨「这次变化是代理做的还是人做的」——是**读**用户输入流，不是注入。
...
- **没有任何对端鉴权**（不校验 peer uid/pid/代码身份，无令牌、无加密）。凡是能以该用户身份打开这个
  socket 的进程，就能等价地驱动 daemon 的全部能力——包括以「GlassPane Daemon」这个已授权主体去操作
  其他 app。信任模型即「本机任意代码 = 代理的手」：跑在你账号下的代码、你接入的 MCP server/插件，
  都在圈内。
- `~/.glasspane/probe.sock` 同源同权限，但**连接方自报 pid**（`hello` 帧字段）。因此同用户进程可以伪装成
  某 app 的探针，注入 handler/state 信号；而探针信号在场时会把归因从 `soft` 升级为 `strong`——即
  **归因强度可被同用户进程伪造**（已知、如实标注，不构成跨用户越权）。
...
- 位置：`~/.glasspane/evidence/<operationId>.json`（一操作一文件，默认归档上限 10000 份）、
  `~/.glasspane/projects.json`、`~/.glasspane/approvals.json`、`~/.glasspane/installer-daemon.log`
  （launchd 的 stdout 与 stderr 都写这里）。
...
- 清理：`glasspaned --evidence-stats` 看体量，`glasspaned --prune-evidence [--older-than <days>]
  [--project <id>] [--dry-run]` 按期限清理，最直接的是停 daemon 后删除 `~/.glasspane/evidence/`。
  审批账本用 `--approval-audit` / `--approval-verify`（SHA-256 链）检查。
...
- 取消持久化：`launchctl bootout gui/$(id -u)/com.glasspane.daemon`，再删除该 plist。
...
- 分发形态是 `codesign --force --sign -`（ad-hoc）+ 不含 cdhash 的 identifier 型 designated requirement，
  目的是让 TCC 席位跨重编译存活、并让系统设置里出现有名字有图标的条目。
...
- 授权主体必须是 daemon（`GlassPane Daemon` / `glasspaned`），不是终端也不是设置窗口——终端能跑不代表
  daemon 能跑，读数不同是真实语义。
- 开发者工具席位无查询接口，面板恒显示「未验证」：不假点亮是要求，不是缺陷。
...
| 版本 | 是否支持 |
|---|---|
| 1.1.x | ✅ 当前版本线，安全修复在此发布 |
| 1.0.x | ❌ 不再修补（版本线已统一到 1.1.0） |
